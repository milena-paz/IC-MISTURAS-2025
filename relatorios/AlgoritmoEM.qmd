...
Suponha que temos uma a.a. $Y_1,..,Y_n \overset{iid}{\sim} \ell st(\mu,\sigma^2,\nu)$, onde $\nu$ é conhecido. Logo, o vetor de parâmetros é dado por $\bm \theta=(\mu,\sigma^2)$. Com a intenção de obter o estimador de máxima verossimilhança para $\bm \theta$, pode-se tentar pelo método tradicional:
...
O que seria muito difícil de se fazer sem utilizar métodos iterativos (como, por exemplo, Newton-Raphson).
:::
...
# 3. Extensões do Algoritmo EM

Note que o exemplo 2.1 foi dado sob a suposição de que $\nu$ era fixo e conhecido, pois caso contrário o processo realizado não seria possível da forma que foi feito. Nem sempre a função $Q$ encontrada terá uma expressão fechada ou computacionalmente simples, o que pode tornar o algoritmo EM inviável.
...
## 3.1 Experança Maximização Condicional (ECM)
...
### 3.1.1 Exemplo com a distribuição T-student Locação-Escala
...
$$
l_c(\bm \theta)=\frac{\nu}{2}\left(\sum^n_{i=1}\log u_i-\sum^n_{i=1}u_i+ n\log (\nu/2) \right)-n\log\big[\Gamma(\nu/2)\big]
\\
-\frac{n}{2}\log\sigma^2-\frac{1}{2\sigma^2}\sum^n_{i=1} u_i(y_i-\mu)^2 + c(\bm u),
$$
...
$$\hat u_i^{(k)}=\frac{\hat\nu^{(k)}+1}{\hat\nu^{(k)}+{d^2_{Mi}}^{(k)})},\quad i=1,2,...,n.$$

$$
\hat\ell_i^{(k)}=\psi[(\hat\nu^{(k)}+1)/2] - \log \left(\frac{\hat\nu^{(k)}+{d^2_{Mi}}^{(k)}}{2}\right),\quad i=1,2,...,n;
$$
...
\log(\hat\nu^{(k+1)}/2)+ 1-\psi(\hat\nu^{(k+1)}/2)+\frac{\sum^n_{i=1}(\hat\ell_i^{(k+1)}-\hat u_i^{(k+1)})}{n}=0
$$
ou seja, que maximize $Q(\hat\theta_2^{(k+1)}|\bm {\hat\theta_1^{(k+1)}})$.
:::
...
## 3.2 Esperança Maximização Condicional "Qualquer" (ECME)

Deriva-se do algoritmo ECM e emprega, além de passos CM, passos CML. A diferença entre essas sub-etapas está na função a ser maximizada: que pode ser tanto a $Q$ quanto a própria log-verossimilhança, ambas condicionadas similarmente ao visto em 3.1. Além disso, é dito ter velocidade de convergência mais rápida. O algoritmo é, então, como segue:

::: {.callout-note appearance="minimal"}
1) **Passo E**: Calcula-se $Q(\bm \theta| \hat{\bm \theta}^{(k)})$;

2) **Passos CM ou CML**: Para cada $i =1,...,S$, se faz uma das opções abaixo:
    a) CM-i: Fixando todos os subvetores $\bm {\hat\theta_j}^{(k)}$, $j \neq i$, busca-se um $\bm \theta_i=\bm{\hat \theta_i}^{(k+1)}$ que maximize $Q(\bm{\theta_i}| \hat{\bm \theta}_{j\neq i}^{(k)})$.
    b) CML-i: Fixando todos os subvetores $\bm {\hat\theta_j}^{(k)}$, $j \neq i$, busca-se um $\bm \theta_i=\bm{\hat \theta_i}^{(k+1)}$ que maximize $l_c(\bm{\theta_i}| \hat{\bm \theta}_{j\neq i}^{(k)})$.
    
:::

### 3.2.1 Exemplo com a distribuição T-student Locação-Escala

Baseando-se no exemplo 3.1.1, podemos substituir o segundo passo CM por um CML. Assim, reescrevendo a log-verossimilhança em função de $\nu$ temos:

$$
l_c(\nu)=
\frac{\nu}{2}\left(\sum^n_{i=1}\log u_i-\sum^n_{i=1}u_i+ n\log (\nu/2) \right)
-n\log\big[\Gamma(\nu/2)\big] + c(\bm u,\bm \theta_1),
$$

onde $c$ é constante em relação a $\nu$. Logo, deseja-se encontrar o ponto crítico:

$$
\frac{dl_c(\nu)}{d\nu}=\frac{1}{2}\sum^n_{i=1}(\log u_i - u_i)+n+\frac{n\log(\nu/2)}{2}-n\psi(\nu/2)=0
$$

$$
\Rightarrow \frac{1}{2n}\sum^n_{i=1}(\log u_i - u_i)+1+\frac{\log(\nu/2)}{2}-\psi(\nu/2)=0
$$

A partir disso, definimos o seguinte algoritmo:
...
- Passo CML-2: Achar um valor $\nu^{(k+1)}$ que seja resolução da equação:
$$
\frac{1}{2n}\sum^n_{i=1}(\log u_i - u_i)+1+\frac{\log(\nu/2)}{2}-\psi(\nu/2)=0
$$
ou seja, que maximize $l_c(\bm{\theta_2}| \hat{\bm \theta}_{1}^{(k+1)})$.
...
```{r}
#reusando mesma amostra do exemplo 2.1 e funções de critério de parada e verossimilhança(!)

CML2 <- function(nu,mu,sig){
  sum(digamma((nu+1)/2) - log((nu+(y-mu)^2/sig)/2)-
        (nu+1)/(nu+(y-mu)^2/sig))/(2*n)+1+log(nu/2)/2-digamma(nu/2)
}
