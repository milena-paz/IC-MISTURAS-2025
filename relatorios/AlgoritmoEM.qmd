...
### Realizando inferência por meio do Algoritmo EM
...
O que seria muito difícil de se fazer sem utilizar métodos iterativos (como, por exemplo, Newton-Raph\-son). Logo, podemos estimar esses parâmetros via Algoritmo EM, pensando no modelo t locação-escala da seguinte maneira:

- $Y_i$ pode ser reescrito da seguinte forma:
$$
Y_i = \mu + U^{-1/2}Z,
$$
onde $Z\sim N(0,\sigma^2)$ e $U\sim \text{Gama}(\nu/2,\nu/2)$.