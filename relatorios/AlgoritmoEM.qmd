...
Cujas funções escore são dadas por:
...
2) **Passos CM ou CML**: Para cada $i =1,...,S$, se faz uma das opções abaixo:
    ...
    b) CML-i: Fixando todos os subvetores $\bm {\hat\theta_j}^{(k)}$, $j \neq i$, busca-se um $\bm \theta_i=\bm{\hat \theta_i}^{(k+1)}$ que maximize $l(\bm{\theta_i}| \hat{\bm \theta}_{j\neq i}^{(k)})$.
    
:::

### 3.2.1 Exemplo com a distribuição T-student Locação-Escala

Baseando-se no exemplo 3.1.1, podemos substituir o segundo passo CM por um CML. Assim, reescrevendo a log-verossimilhança de $Y$ em função de $\nu$ temos:

$$
\begin{aligned}
l(\nu)&\propto n\log [k(\nu)] -\frac{\nu+1}{2}\sum^n_{i=1}\log\left(1+d^2_{Mi}\nu^{-1}\right)
\\
&\propto n\log\left[\frac{\Gamma\left(\frac{\nu+1}{2}\right)}{\Gamma\left(\frac{\nu}{2}\right)\sqrt{\pi\nu}}\right]-\frac{\nu+1}{2}\sum^n_{i=1}\log\left(1+\frac{(y_i-\mu)^2}{\sigma^2\nu}\right)
\end{aligned}
$$

Logo, deseja-se encontrar o ponto crítico:

$$
\frac{dl(\nu)}{d\nu}=n/2\left\{\psi[(\nu+1)/2]-\psi(\nu/2)-\nu^{-1}\right\}-
\frac{1}{2}\sum^n_{i=1}\log\left(1+d^2_{Mi}\nu^{-1}\right)+
\frac{\nu+1}{2\nu}\sum^n_{i=1}\frac{d^2_{Mi}}{\nu+d^2_{Mi}}
$$

$$
\Rightarrow n\left\{\psi[(\nu+1)/2]-\psi(\nu/2)-\nu^{-1}\right\}-
\sum^n_{i=1}\log\left(1+d^2_{Mi}\nu^{-1}\right)+
\frac{\nu+1}{\nu}\sum^n_{i=1}\frac{d^2_{Mi}}{\nu+d^2_{Mi}}=0
$$
...
\frac{\nu+1}{\nu}\sum^n_{i=1}\frac{d^2_{Mi}}{\nu+d^2_{Mi}}=0
$$
ou seja, que maximize $l(\bm{\theta_2}| \hat{\bm \theta}_1^{(k+1)})$.
