...
*
$$
\left. \frac{dQ(\bm \theta| \hat{\bm \theta})}{d\sigma} \right\vert^{{\bm \sigma^2}={\bm{\hat \sigma^2}^{(k+1)}};\bm{\hat \theta}=\bm{\hat \theta^{(k)}}}=
\left. -\frac{n}{2\sigma^2}+\frac{1}{2(\sigma^2)^2}\sum^n_{i=1}u_i(y_i-\mu)^2\right\vert^{{\bm \sigma^2}={\bm{\hat \sigma^2}^{(k+1)}};\bm{\hat \theta}=\bm{\hat \theta^{(k)}}}=0
$$

$$
\Rightarrow {\hat\sigma^2}^{(k+1)}=\frac{\sum^n_{i=1}\hat u_i^{(k)}(y_i-{\hat\mu}^{(k)})^2}{n}
$$ {#eq-sigma2}
...
$$\hat u_i=\frac{\nu+1}{\nu+d^2_{Mi}(y_i,\hat\mu,\hat\sigma^2)},\quad i=1,2,...,n;$$
