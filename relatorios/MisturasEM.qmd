...
$$
\frac{dQ(\bm \Psi|\bm{\Psi^{(k)}})}{d\mu_j}=\sum^n_{i=1}\hat z_{ij}\frac{(y_i-\mu_j)}{\sigma^2_j}
$$

$$
\Rightarrow \frac{\sum^n_{i=1}z_{ij}^{(k)}(y_i-\mu_j^{(k+1)})}{{\sigma^2_j}^{(k)}}=0
\Rightarrow \sum^n_{i=1}\hat z_{ij}^{(k)}y_i=\sum^n_{i=1}\hat z_{ij}^{(k)}\mu_j^{(k+1)}
$$

$$
\Rightarrow
\mu_j^{(k+1)}=\frac{\sum^n_{i=1}\hat z_{ij}^{(k)}y_i}{\sum^n_{i=1}\hat z_{ij}^{(k)}}
$$ {#eq-mu}

$$
\frac{dQ(\bm \Psi|\bm{\Psi^{(k)}})}{d\sigma^2_j}=
\sum^n_{i=1}z_{ij}\left[-\frac{1}{2\sigma^2_j}+\frac{(y_i-\mu_j)^2}{2({\sigma^2_j})^2}\right]
$$

$$
\Rightarrow \sum^n_{i=1}z_{ij}^{(k)}\left[-\frac{1}{2{\sigma^2_j}^{(k+1)}}+\frac{(y_i-\mu_j^{(k+ 1)})^2}{2({\sigma^2_j}^{(k+1)})^2}\right]=0\Rightarrow
\frac{\sum^n_{i=1}z_{ij}^{(k)}(y_i-\mu_j^{(k+1)})^2}{{\sigma^2_j}^{(k+1)}}=\sum^n_{i=1}z_{ij}^{(k)}
$$

$$
\Rightarrow {\sigma^2_j}^{(k+1)}=\frac{\sum^n_{i=1}z_{ij}^{(k)}(y_i-\mu_j^{(k)})^2}{\sum^n_{i=1}z_{ij}^{(k)}}
$$ {#eq-sigma}

Assim temos o seguinte algoritmo:

::: {.callout-note}
1) **Passo E**: Calcula-se $\hat z_{ij}=\frac{p_{j}^{(k)} f_j(y_i|\mu_j^{(k)},{\sigma_j^2}^{(k)})}{\sum^g_{h=1} p_h^{(k)}f_h(y_i|\mu_j^{(k)},{\sigma_j^2}^{(k)})}$;

2) **Passo M**: Obtem-se $\bm \mu^{(k+1)},\bm {\sigma^2}^{(k+1)}$ que maximizem a função $Q$, ou seja, calculam-se os pontos críticos da @eq-mu e @eq-sigma.
